...
import ConsultoraClientes from './pages/ConsultoraClientes';
import Equipe from './pages/Equipe';
import Relatorios from './pages/Relatorios';
import Ranking from './pages/Ranking';
import { Columns, LayoutDashboard, ReceiptText, Users, Settings, UserCog, BarChart3, Trophy, Sun, Moon, Briefcase } from 'lucide-react';

function Sidebar({ theme, toggleTheme, role }) {
      ...
      <nav className="nav-links mt-8">
        ...
        {isAdmin ? (
          ...
          <>
            ...
            <Link to="/meus-clientes" className={`nav-item ${location.pathname === '/meus-clientes' ? 'active' : ''}`}>
              <Users size={20} />
              Meus Clientes
            </Link>
            <Link to="/ranking" className={`nav-item ${location.pathname === '/ranking' ? 'active' : ''}`}>
  ...
  return (
    <BrowserRouter>
      <div className="app-container">
        <Sidebar theme={theme} toggleTheme={toggleTheme} role={user.role} />
        <main className="main-content">
          <Routes>
            {isAdmin ? (
              ...
              <>
                <Route path="/" element={<ConsultoraPainel user={user} onLogout={() => setUser(null)} theme={theme} toggleTheme={toggleTheme} />} />
                <Route path="/meus-clientes" element={<ConsultoraClientes user={user} />} />
                <Route path="/ranking" element={<Ranking />} />
                <Route path="*" element={<ConsultoraPainel user={user} onLogout={() => setUser(null)} theme={theme} toggleTheme={toggleTheme} />} />
              </>
            )}
